...
from mymodels import Participant
...
def setup():
    ...
    #
    #        #cc = partner.counterbalance
    #        #dstr = partner.datastring
    #        #ind = dstr.find('gender')
    #        #if ind!=-1:
    #        #    sex = dstr[(ind+9):(ind+10)]
    #        #    if sex in ['m', 'f']:
    #        #        cb_by_sex[sex][cc] += 1

    #partner_ids.sort()

    # get counterbalance distribution by sex
    #q = Participant.query.\
    #    filter(Participant.status > 1, Participant.sex != None).\
    #    all()

    #cb_by_sex = {'male': [0, 0, 0, 0], 'female': [0, 0, 0, 0]}

    #for rec in q:
    #    pid = str(rec.workerid)
    #    if pid.count('-retest') == 0 and pid.count('throwaway') == 0:

    #        cc = rec.counterbalance
    #        sex = rec.sex
...
@custom_code.route('/counters', methods=['GET'])
def get_counterbalance_dist():
    current_app.logger.info("Reached /counters")  # Print message to server.log for debugging
    ...
    print cb_by_sex
    return jsonify({'cb_by_sex': cb_by_sex})
